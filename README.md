...
The four mp3 files in `audio/test1/` are now generated with a
text-to-speech engine (espeak-ng), not silence. This means the test is
fully playable end to end and the questions genuinely match what's
said, but the voice is robotic, not a natural human voice, and IELTS
listening tests use real speakers with natural intonation and a mix of
accents. Treat this as a working stand-in your students can practise
the mechanics on, not a substitute for a proper recording before it
goes out for real practice. When you're ready to upgrade the audio:

- **Record it yourself** — read the scripts in `generate_audio.py`
  (or your own rewritten versions) with colleagues playing the
  different speakers, using a phone voice recorder.
- **Use a natural-sounding TTS service** — Google Cloud Text-to-Speech,
  Amazon Polly, ElevenLabs, or Microsoft Azure all produce much more
  natural voices than espeak-ng and have free tiers.
- **Hire voice talent** — sites like Fiverr have affordable options for
  short recordings like this.

Whichever route you take, just export as mp3 and drop it into
`audio/test1/` with the same filenames.
